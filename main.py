...
filename = 'output.bin'
#address = 0x8  # Dirección donde se desea escribir (en este caso, posición 16 en decimal)
#data = 0x22334455  # Dato a escribir


def write_to_specific_address(filename, address, data):
    # Abre el archivo en modo binario de escritura y lectura
    with open(filename, 'r+b') as f:
        # Mueve el puntero del archivo a la dirección especificada
        f.seek(address)
        
        # Convierte el dato a bytes (aquí asumo que es un entero de 4 bytes)  big o little
        data_bytes = data.to_bytes(4, byteorder='big')
        
        # Escribe el dato en la posición actual del puntero
        f.write(data_bytes)
...
if __name__ == '__main__':
    ...
    for address in range(start_address, end_address, WORD_SIZE):
        ...
        if binary_output:
            ...
            write_to_specific_address(filename, address, recovered_value)
